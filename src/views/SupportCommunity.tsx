...
export const SupportCommunity: React.FC = () => {
  ...
  const [userTickets, setUserTickets] = useState<SupportTicket[]>(() => {
    try {
      const saved = localStorage.getItem(getUserTicketsKey(activeUserId));
      if (saved) {
  ...
  const handleCreateTicket = (e: React.FormEvent) => {
    ...
    localStorage.setItem(getUserTicketsKey(activeUserId), JSON.stringify(updated));
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { title: '1-on-1 Call', desc: 'VIP Concierge for Pro & VIP Tiers', icon: PhoneCall, action: 'Book Strategy', color: 'bg-amber-50 text-amber-600' },
            ].map((ch) => {
              const Icon = ch.icon;
              return (
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-card overflow-hidden">
            ...
            {userTickets.length === 0 ? (
              <div className="p-10 text-center bg-slate-50/50">
                ...
                <h5 className="text-sm font-bold text-slate-800">No Support Tickets</h5>
                <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1 mb-4">
                  Need help with domain integration, payouts, or binary genealogy? Create a ticket and our team will assist you.
                </p>
                <button
                  onClick={() => setShowNewTicketModal(true)}
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-card space-y-3 text-xs">
            ...
            <div className="space-y-2 pt-1">
              {[
                ...
                <div key={s.service} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-100">
                  <div>
                    <p className="font-bold text-slate-800">{s.service}</p>
                    <span className="text-[10px] text-slate-400 font-mono">Response: {s.latency}</span>
                  </div>
            ...
            <form onSubmit={handleCreateTicket} className="space-y-3">
              <div>
                ...
                >
                  ...
                  <option>Wallet Operations & Payouts</option>
                  <option>Binary Network & Genealogy</option>
                  <option>Marketplace & Seller Hub</option>
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Domain SSL configuration question"
                  value={ticketSubject}
